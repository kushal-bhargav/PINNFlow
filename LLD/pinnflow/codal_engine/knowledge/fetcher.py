...
class CodalFetcher:
    ...
    def __init__(self, storage_path: str = "data/standards"):
        ...
        self.storage_path = storage if storage.is_absolute() else self.repo_root / storage
        self.storage_path.mkdir(parents=True, exist_ok=True)
        # Directory where extracted text is cached as .txt files to avoid
        # re-running expensive OCR or PDF parsing on every startup.
        self.cache_dir = self.storage_path / "cache"
    ...
    def _cache_path(self, target: Path) -> Path:
        """Derive a stable cache filename from the standards document identity."""
        key = hashlib.md5(target.name.lower().encode()).hexdigest()[:12]
        return self.cache_dir / f"{target.stem}_{key}.txt"
    ...
    def _load_cache(self, target: Path) -> str | None:
        ...
        if self.cache_dir.exists():
            for stem in dict.fromkeys(search_stems):
                for candidate in sorted(self.cache_dir.glob(f"{stem}_*.txt")):
                    text = candidate.read_text(encoding="utf-8", errors="ignore")
                    if text.strip():
                        print(f"[Codal] Using existing cache file for {target.name}: {candidate.name}")
                        self._save_cache(target, text)
                        if candidate != cache_file and cache_file.exists():
                            try:
                                candidate.unlink()
                                print(f"[Codal] Removed legacy duplicate cache file: {candidate.name}")
                            except Exception as exc:
                                print(f"[Codal] Warning: could not remove legacy cache {candidate.name}: {exc}")
                        return text
